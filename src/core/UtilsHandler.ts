...
class UtilsHandler {
    ...
    findById = (id: string): WorkareaObject | null => {
        let findObject = null
        const exist = this.handler.canvas.getObjects().some((obj: WorkareaObject) => {
            if (obj.id === id) {
                findObject = obj
                return true
            }
    ...
    fileUpload = async (file: File, name: string, type: string) => {
        ...
        try {
            ...
            const options: any = {
                ...
                src,
                // 添加高质量渲染选项
                imageSmoothing: true, // 启用高质量图片平滑
                preserveAspectRatio: true, // 保持宽高比
                crossOrigin: 'anonymous',
                objectCaching: false, // 禁用缓存以确保质量
                perPixelTargetFind: true // 启用像素级精确渲染
            }
